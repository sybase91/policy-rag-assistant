...
A beginner-friendly but enterprise-oriented RAG (Retrieval-Augmented Generation) prototype that answers questions from public AI governance and LLM security PDFs — with source citations, grounded responses, and a local Streamlit chat UI.

**Status:** Phases 1–5 complete. Phase 6 (evaluation) is next.
...
- A clear path toward **evaluation**, quality measurement, and later architecture upgrades
...
| **Evaluation TODO** | Measure whether retrieval and answers are actually good (Phase 6). |
...
| **Phase 6 — Basic RAG evaluation** | Planned / Next | `gold_questions.csv`; evaluation script; source hit rate; keyword hit rate; refusal accuracy; pass rate; evaluation report | Moves the project from “it works” to “we can measure whether it works” | `src/evaluate.py`, `evals/gold_questions.csv`, `evals/eval_results.csv`, `reports/evaluation_report.md` | `python -m src.evaluate` *(planned)* |

### Phase highlights (quick read)

**Phase 1** prepares trustworthy source units. **Phase 2** makes them searchable. **Phase 3** finds evidence. **Phase 4** writes grounded answers. **Phase 5** makes the system showcaseable in a browser. **Phase 6** will score quality systematically.
...
**Never commit `.env`.** It is ignored by `.gitignore`. Phases 2–5 require a real key for OpenAI embeddings and chat.
...
Try these in the Streamlit UI or via `python -m src.generate`:
...
- Evaluation is still planned (Phase 6)

---

## 11. Roadmap

1. **Phase 6** — Basic RAG evaluation (`gold_questions.csv`, metrics, evaluation report)
2. **Phase 7** — README polish, screenshots, demo recording
3. **Phase 8** — Retrieval quality improvements
4. **Phase 9** — Reranking / hybrid search
5. **Phase 10** — Deployment
6. **Phase 11** — FastAPI + Next.js version
7. **Phase 12** — Lead-capture website integration
...
The intent is to demonstrate not only “I can call an LLM,” but also how to design a system leaders can trust: grounded retrieval, citation metadata, clear limitations, and a path to measurable quality.
